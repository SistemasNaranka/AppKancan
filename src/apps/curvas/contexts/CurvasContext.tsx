...
import { saveLogCurvas, saveLogsBatch, saveEnviosBatch } from '../api/directus/create';
import { getLogCurvas, getTiendas } from '../api/directus/read';
import { obtenerBloqueosActivos, intentarBloquearTienda, liberarTodosLosBloqueosDeUsuario, liberarTienda } from '../api/directus/bloqueos';
...
interface CurvasContextType {
  ...
  limpiarDatos: () => void;
  cargarDatosGuardados: () => Promise<void>;

  // Acciones de edición (solo Admin)
  editarCelda: (sheetId: string, filaId: string, columna: string, valorNuevo: number | string) => void;
  guardarCambios: (datosLog?: { tiendaId: string; tiendaNombre: string; plantilla: 'matriz_general' | 'productos'; cantidadTalla: { talla: number; cantidad: number }[]; referencia?: string; estado?: 'borrador' | 'confirmado' }[]) => Promise<boolean>;
  guardarLogCurvas: (data: { tiendaId: string; tiendaNombre: string; plantilla: 'matriz_general' | 'productos'; cantidadTalla: { talla: number; cantidad: number }[]; referencia?: string; estado?: 'borrador' | 'confirmado' }) => Promise<boolean>;
  confirmarLote: (tipo: 'general' | 'producto_a' | 'producto_b', sheetId: string) => Promise<boolean>;
  descartarCambios: () => void;
  setHasChanges: (val: boolean) => void;

  // Acciones de validación (Despacho)
  actualizarValorValidacion: (sheetId: string, filaId: string, columna: string, valor: number) => void;
  limpiarValidacion: (sheetId?: string) => void;
  guardarEnvioDespacho: (
    sheetId: string, 
    overrideData?: Record<string, Record<string, number>>, 
    overridePlantilla?: 'matriz_general' | 'productos',
    overrideRef?: string
  ) => Promise<boolean>;
  validationData: Record<string, Record<string, Record<string, number>>>;
...
export const CurvasProvider = ({ children }: { children: ReactNode }) => {
  ...
  const intentarBloquear = useCallback(async (tienda_id: string, referencia: string) => {
  ...
  const desmarcarTienda = useCallback(async (tienda_id: string, referencia: string) => {
    if (!user) return;
  ...
  const [validationData, setValidationData] = useState<Record<string, Record<string, Record<string, number>>>>({}); // { sheetId: { filaId: { col: valor } } }
  ...
  const procesarArchivo = useCallback(async (file: File, tipo: ArchivoSubido['tipo']) => {
    ...
    try {
      ...
      setArchivos(prev => prev.map(a =>
        a.id === archivoId ? { ...a, estado: 'procesando' as EstadoCarga, progreso: 50 } : a
      ));
  ...
  const cargarDatosManuales = useCallback((data: MatrizGeneralCurvas | DetalleProducto, tipo: ArchivoSubido['tipo']) => {
  ...
  const cargarDatosGuardados = useCallback(async () => {
    try {
      ...
      logs.forEach(log => {
        ...
        const logExistente = groups[groupKey].logs.find(l => l.tienda_id === log.tienda_id);
        if (!logExistente) {
      ...
      Object.values(groups).forEach(group => {
        ...
        group.logs.forEach(log => {
          ...
          try {
            cantidadTalla = typeof log.cantidad_talla === 'string' ? JSON.parse(log.cantidad_talla) : log.cantidad_talla;
          } catch (e) {
            console.error('Error parseando cantidad_talla en log:', log.id, e);
          }
          ...
          const fila = {
            id: String(log.tienda_id),
            tienda: {
              id: log.tienda_id,
              nombre: tiendasDict[log.tienda_id] || log.tienda_nombre || 'Tienda ' + log.tienda_id,
              codigo: ''
            },
            [group.plantilla === 'matriz_general' ? 'curvas' : 'tallas']: rowData,
            total: rowTotal
          };
          filas.push(fila);
        });
        ...
        sortedColumns.forEach(col => {
          colTotals[col] = filas.reduce((sum, f) => {
            const data = group.plantilla === 'matriz_general' ? f.curvas : f.tallas;
        ...
        if (group.plantilla === 'matriz_general') {
          ...
          productos.push({
            ...
            metadatos: {
              ...
              imagen: '',
              color: group.referencia.includes('|') ? group.referencia.split('|')[1].trim() : '',
              proveedor: group.logs[0]?.proveedor || 'RECUPERADO',
              precio: group.logs[0]?.precio || 0,
              linea: group.plantilla === 'productos' ? (group.referencia.includes('|') ? 'CALZADO' : 'PRODUCIDO') : 'GENERAL',
            },
      ...
      setDatosCurvas(prev => {
        ...
        prev.matrizGeneral.forEach(oldSheet => {
          ...
          const existsInDB = matrizGeneral.some(dbSheet =>
            extractRef(dbSheet).toUpperCase() === oldRef
          );
        ...
        prev.productos.forEach(oldSheet => {
          const oldRef = extractRef(oldSheet).toUpperCase();
          const existsInDB = productos.some(dbSheet =>
            extractRef(dbSheet).toUpperCase() === oldRef
          );
  ...
  }, [cargarDatosGuardados, lastLogsUpdate]);
  ...
  const extractRef = useCallback((sheet: any) => {
    ...
    if (sheet.id && typeof sheet.id === 'string') {
      ...
      if (fromId && fromId.toUpperCase() !== 'NUEVA' && fromId.toUpperCase() !== 'SIN REF') {
        return fromId;
      }
    }
    ...
    const desperateFallback = candidates
      ...
      .find(c => c !== '' && !c.includes('Manual -') && !c.startsWith('sheet-'));

    return desperateFallback || 'SIN REF';
  }, []);

  const editarCelda = useCallback((sheetId: string, filaId: string, columna: string, valorNuevo: number | string) => {
      ...
      if (isGeneral) {
        const updatedData = prev.matrizGeneral.map(sheet => {
          ...
          const nuevasFilas = sheet.filas.map(fila => {
            ...
            if (typeof valorNuevo === 'number') {
              nuevasCurvas[columna] = {
                ...(nuevasCurvas[columna] || { valor: 0, id: `${filaId}-${columna}` }),
                valor: valorNuevo,
                esCero: valorNuevo === 0,
                esMayorQueCero: valorNuevo > 0
              };
            }
            const nuevoTotal = Object.values(nuevasCurvas).reduce((sum, c) => sum + c.valor, 0);
          ...
          sheet.curvas.forEach(c => {
            nuevosTotalesPorCurva[c] = nuevasFilas.reduce((sum, f) => sum + (f.curvas[c]?.valor || 0), 0);
          });
        ...
        const updatedData = prev.productos.map(sheet => {
          ...
          const nuevasFilas = sheet.filas.map(fila => {
            ...
            if (typeof valorNuevo === 'number') {
              nuevasTallas[columna] = {
                ...(nuevasTallas[columna] || { valor: 0, id: `${filaId}-${columna}` }),
                valor: valorNuevo,
                esCero: valorNuevo === 0,
                esMayorQueCero: valorNuevo > 0
              };
            }
            const nuevoTotal = Object.values(nuevasTallas).reduce((sum, c) => sum + c.valor, 0);
          ...
          sheet.tallas.forEach(t => {
            nuevosTotalesPorTalla[t] = nuevasFilas.reduce((sum, f) => sum + (f.tallas[t]?.valor || 0), 0);
          });
  ...
  const guardarCambios = useCallback(async (
    ...
    if (datosLog && datosLog.length > 0) {
      ...
      affectedSheets.forEach(sheetId => {
        ...
        const sheet = datosCurvas.matrizGeneral.find(s => s.id === sheetId) ||
    ...
    try {
      ...
      const isActuallyAdmin = userRole.toLowerCase().includes('admin') ||
  ...
  const confirmarLote = useCallback(async (tipo: 'general' | 'producto_a' | 'producto_b', sheetId: string): Promise<boolean> => {
    ...
    try {
      ...
      if (tipo === 'general') {
        sheetToPersist = datosCurvas.matrizGeneral.find(s => s.id === sheetId);
      } else {
        sheetToPersist = datosCurvas.productos.find(s => s.id === sheetId);
      }
      ...
      setDatosCurvas(prev => {
        if (!prev) return prev;
        if (tipo === 'general') {
          const matrizGeneral = prev.matrizGeneral.map(s => s.id === sheetId ? { ...s, estado: 'confirmado' as const } : s);
          return { ...prev, matrizGeneral };
        } else {
          const updated = prev.productos.map(s => s.id === sheetId ? { ...s, estado: 'confirmado' as const } : s);
  ...
  // ============================================
  const cargarDatosPorFecha = useCallback(async (fecha: string) => {
    try {
      ...
      logs.forEach(log => {
        ...
        const exists = groups[groupKey].logs.find(l => l.tienda_id === log.tienda_id);
  ...
  // ============================================
  const reutilizarLote = useCallback(async (sheetId: string): Promise<boolean> => {
    if (!datosCurvas || !user) return false;
    try {
      let sheet: any = datosCurvas.matrizGeneral.find(s => s.id === sheetId)
        || datosCurvas.productos.find(s => s.id === sheetId);
      ...
      const okLogs = await saveLogsBatch(logsBatch);

      if (!okLogs) throw new Error('Error guardando reutilización');
  ...
  const guardarLogCurvas = useCallback(async (
    data: {
      tiendaId: string;
      tiendaNombre: string;
      plantilla: 'matriz_general' | 'productos';
  ...
  const agregarArticuloEscaneado = useCallback((articulo: Omit<ArticuloEscaneado, 'id' | 'fechaEscaneo'>) => {
  ...
  const crearEnvio = useCallback((tiendaId: string): EnvioTienda | null => {
  ...
  const actualizarEstadoEnvio = useCallback((envioId: string, estado: EnvioTienda['estado']) => {
    ...
    setEnvios(prev => prev.map(e =>
      e.id === envioId ? {
        ...e,
        estado,
        fechaDespacho: estado === 'despachado' ? new Date() : e.fechaDespacho,
        usuarioDespacho: estado === 'despachado' ? userRole : e.usuarioDespacho,
      } : e
    ));
  ...
  const actualizarValorValidacion = useCallback((sheetId: string, filaId: string, col: string, valor: number) => {
    setValidationData(prev => {
      const next = { ...prev };
      const sId = String(sheetId);
      if (!next[sId]) next[sId] = {};
      if (!next[sId][filaId]) next[sId][filaId] = {};
      next[sId][filaId][col] = valor;
      return next;
    });
  }, []);

  const limpiarValidacion = useCallback((sheetId?: string) => {
    if (user) {
  ...
  const guardarEnvioDespacho = useCallback(async (
    sheetId: string, 
    overrideData?: Record<string, Record<string, number>>,
    overridePlantilla?: 'matriz_general' | 'productos',
    overrideRef?: string
    ...
    if (Object.keys(currentSheetValidation).length === 0) {
      console.warn('⚠️ [guardarEnvioDespacho] No hay datos de validación para enviar (lote vacío)');
      return false;
    }

    try {
      // 1. Intentar encontrar la hoja original (plantilla)
      let template: any = [...(datosCurvas?.matrizGeneral || []), ...(datosCurvas?.productos || [])].find(s =>
      ...
      const refFinal = overrideRef || (template ? extractRef(template) : sheetIdStr);
      const isMatriz = overridePlantilla === 'matriz_general' || 
                       (template ? datosCurvas?.matrizGeneral?.some(s => s.id === template.id) : (refFinal.length < 6));
      const plantilla: 'matriz_general' | 'productos' = isMatriz ? 'matriz_general' : 'productos';
      const fechaActual = new Date().toISOString();
      ...
      for (const [filaId, tallas] of Object.entries(currentSheetValidation)) {
        ...
        const fila = template?.filas?.find((f: any) =>
          String(f.id) === String(filaId) ||
          String(f.tienda?.id) === String(filaId) ||
          String(f.tienda?.codigo) === String(filaId)
        );

        const tiendaIdFinal = fila?.tienda?.id || (tiendasDict[filaId] ? filaId : null);

        if (!tiendaIdFinal) {
          console.warn(`❓ [guardarEnvioDespacho] No se pudo resolver ID de tienda para la fila ${filaId}. Saltando...`);
          continue;
        }

        const cantidadTalla: { talla: number; cantidad: number }[] = Object.entries(tallas)
          .filter(([_, qty]) => qty > 0)
          .map(([talla, qty]) => ({
      ...
      if (enviosBatch.length === 0) {
        console.warn('⚠️ [guardarEnvioDespacho] El lote de envío está vacío tras procesar tiendas');
        return true; 
      }
      ...
      if (success) {
        ...
        console.error('❌ [guardarEnvioDespacho] El servidor rechazó el batch de envío');
        return false;
      }
    } catch (err) {
  ...
  }, [user, validationData, datosCurvas, tiendasDict, extractRef, limpiarValidacion]);

  // ============================================
  // ACCIONES DE COMPARACIÓN
  // ============================================

  const generarComparacion = useCallback((envioId: string): ComparacionEnvio[] => {
  ...
  const generarReporteDiscrepancias = useCallback((envioId: string): ReporteDiscrepancias | null => {
    const envio = envios.find(e => e.id === envioId);
  ...
  const value: CurvasContextType = useMemo(() => ({
    ...
    internalRole, permissions, setUserRole, datosCurvas, archivos, celdasEditadas,
    hasChanges, envios, articulosEscaneados, procesarArchivo, limpiarDatos,
    cargarDatosGuardados, editarCelda, guardarCambios, guardarLogCurvas,
    confirmarLote, descartarCambios, agregarArticuloEscaneado, crearEnvio,
    actualizarEstadoEnvio, generarComparacion, generarReporteDiscrepancias,
    actualizarValorValidacion, limpiarValidacion, guardarEnvioDespacho,
    cargarDatosManuales, setHasChanges, refreshLogs, lastLogsUpdate,
    bloqueosActivos, intentarBloquear, desmarcarTienda,
    tiendasDict, extractRef, cargarDatosPorFecha, reutilizarLote,
    validationData,
  ]);

  return (
    <CurvasContext.Provider value={value}>
      {children}
    </CurvasContext.Provider>
  );
};
