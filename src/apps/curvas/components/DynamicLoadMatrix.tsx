...
const DynamicLoadMatrix = forwardRef<DynamicLoadMatrixHandle, DynamicLoadMatrixProps>(({ onCancel, onChange, type }, ref) => {
    ...
    const [columns, setColumns] = useState<string[]>(['35', '36', '37', '38', '39', '40']);
    ...
    const [selection, setSelection] = useState<{
        startR: number, startCIdx: number,
        endR: number, endCIdx: number,
        isSelecting: boolean
    } | null>(null);
    ...
    useEffect(() => {
        ...
        if (tiendasLista && tiendasLista.length > 0) {
            ...
            setRows(defaultTiendas.map((t: Tienda) => ({
                id: t.id,
                name: t.nombre,
                tiendaData: t,
                values: {}
            })));
        } else if (rows.length === 0 || (rows.length === 40 && rows[0].name === '')) {
            // Solo poner filas vacías si no hay tiendas (posiblemente aún cargando)
            setRows(Array.from({ length: 40 }, (_, i) => ({
                id: (i + 1).toString(),
                name: '',
                values: {}
            })));
    ...
    const columnTotals = useMemo(() => {
        ...
        columns.forEach(col => {
            totals[col] = rows.reduce((acc, row) => acc + (Number(row.values[col]) || 0), 0);
        });
        return totals;
    }, [rows, columns]);
    ...
    const handleValueChange = (rowIndex: number, colName: string, value: string) => {
    ...
    const addColumn = () => {
        ...
        const nextCol = !isNaN(Number(lastCol)) ? (Number(lastCol) + 1).toString() : 'NUEVA';
    ...
    const handlePaste = (e: React.ClipboardEvent, startRowIndex: number, startColName: string) => {
        e.preventDefault();
        ...
        const isFirstValueNumeric = firstLineValues.length > 0 && !isNaN(Number(firstLineValues[0].trim()));
        ...
        lines.forEach((line, lineIdx) => {
            ...
            if (!isFirstValueNumeric) {
                ...
                const matchedStore = tiendasLista.find(t =>
                    t.nombre.toLowerCase() === firstColValue.toLowerCase() ||
                    t.codigo.toLowerCase() === firstColValue.toLowerCase()
                );

                if (matchedStore) {
                    const existingRowIdx = newRows.findIndex(r => r.name === matchedStore.nombre);
                    if (existingRowIdx !== -1) {
            ...
            valuesToPaste.forEach((val, valIdx) => {
                ...
                if (targetColIdx >= tempColumns.length) {
                    ...
                    const nextVal = !isNaN(Number(lastCol)) ? (Number(lastCol) + 1).toString() : (tempColumns.length + 1).toString();
    ...
    useEffect(() => {
        ...
        debounceTimerRef.current = setTimeout(() => {
            ...
            const payload = {
                ...
                filas: rows
                    .filter(r => {
                        const rowTotal = Object.values(r.values).reduce((sum: number, v) => sum + (Number(v) || 0), 0 as number);
                    ...
                    .map(r => ({
                        ...
                        [dataKey]: Object.entries(r.values).reduce((acc, [k, v]) => ({
                            ...acc,
                            [k]: { valor: Number(v) || 0, esCero: (Number(v) || 0) === 0, esMayorQueCero: (Number(v) || 0) > 0 }
                        }), {}),
                        total: Object.values(r.values).reduce((sum: number, v: any) => sum + (Number(v) || 0), 0)
                    })),
                [isGeneral ? 'curvas' : 'tallas']: columns,
                [totalsKey]: columns.reduce((acc, c) => ({
                    ...acc,
                ...
                <Table stickyHeader size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            {columns.map((col, idx) => {
                                ...
                                const isColActive = activeCell?.c === col || (idx >= minC && idx <= maxC);
                                return (
                                    <TableCell
                                        key={col}
                                        align="center"
                                        sx={{
                                            ...
                                            color: '#fff',
                                            boxShadow: isColActive ? 'inset 0 -4px 0 rgba(255,255,255,0.8)' : 'none',
                                    ...
                                    >
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
                                            {col}
                                            <IconButton size="small" onClick={() => removeColumn(col)} sx={{ color: alpha('#fff', 0.4), '&:hover': { color: '#ef4444' } }}>
                    ...
                    <TableBody>
                        ...
                        <TableRow sx={{ position: 'sticky', bottom: 0, zIndex: 10 }}>
                            ...
                            {columns.map(col => (
                                <TableCell key={col} align="center" sx={{ bgcolor: '#f1f5f9', color: '#4f46e5', fontWeight: 900 }}>
                                    {columnTotals[col] || 0}
                                </TableCell>
                            ))}
                            <TableCell align="center" sx={{
            ...
            >
                ...
                <DialogContent>
                    <Typography variant="body1" sx={{ color: '#334155', mt: 1 }}>
                        Los datos superan las columnas actuales. ¿Deseas crear las {(pendingPasteData?.newColumns.length || 0) - columns.length} columnas adicionales automáticamente?
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ p: 2.5, pt: 0 }}>
                    <Button onClick={() => setConfirmPasteOpen(false)} sx={{ fontWeight: 700, px: 3 }}>Cancelar</Button>
                    <Button onClick={confirmAutoColumns} variant="contained" sx={{ borderRadius: 2, fontWeight: 900, px: 4, bgcolor: '#4f46e5' }}>Sí, Crear y Pegar</Button>
                </DialogActions>
            </Dialog>
