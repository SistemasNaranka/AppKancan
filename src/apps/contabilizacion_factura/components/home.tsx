...
const ESTADO_CONFIG: Record<EstadoProceso, { label: string; color: 'info' | 'warning' | 'success' | 'error' | 'default' }> = {
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            >
                ...
                >
                    ...
                    >
                        <Chip
                            icon={<CalendarToday sx={{ color: 'inherit !important', fontSize: 16 }} />}
                        ...
                        <Table size="small">
                            <TableHead>
                                <TableRow sx={{ backgroundColor: 'grey.50' }}>
                                    ...
                                    <TableCell align="right" sx={{ fontWeight: 600 }}>P. Unit.</TableCell>
                            ...
                            <TableBody>
                                {datosFactura.conceptos.map((concepto, index) => (
                                    ...
                                    >
                                        ...
                                        <TableCell align="right">
                                            <Typography variant="body2">
                                                {formatCurrency(concepto.precioUnitario, datosFactura.moneda)}
                                            </Typography>
                                        </TableCell>
                                        <TableCell align="right">
                                            <Typography
            ...
            >
                ...
                >
                    ...
                    <Box sx={{ p: { xs: 2, sm: 3 }, borderRight: { md: '1px solid' }, borderColor: 'grey.200' }}>
                                ...
                                {datosFactura.impuestos.map((impuesto, index) => (
                                    <Box
                                        ...
                                        sx={{
                                            ...
                                            justifyContent: 'space-between',
                                            py: 1,
                                            borderBottom: index < datosFactura.impuestos.length - 1 ? '1px solid' : 'none',
 ...
 */
function ProcessingFeedback({ message, progress }: { message: string; progress: number }) {
    return (
        <Box
            sx={{
                display: 'flex',
...
function ErrorDisplay({ error, onRetry, onClear }: { 
    ...
    const errorMessages: Record<TipoErrorPDF, { title: string; message: string; suggestion: string }> = {
        archivo_invalido: {
            title: 'Archivo inválido',
            message: error.mensaje,
            suggestion: 'Verifica que el archivo sea un PDF válido y no esté corrupto.',
        },
        pdf_protegido: {
            title: 'PDF protegido',
            message: error.mensaje,
            suggestion: 'El documento está protegido con contraseña. Desprotégelo antes de subirlo.',
        },
        extraccion_fallida: {
            title: 'Error de extracción',
            message: error.mensaje,
            suggestion: 'El PDF puede tener un formato no estándar. Intenta guardarlo nuevamente.',
        },
        formato_no_reconocido: {
            title: 'Formato no reconocido',
            message: error.mensaje,
            suggestion: 'El documento no parece ser una factura estándar. Verifica el archivo.',
        },
        datos_incompletos: {
            title: 'Datos incompletos',
            message: error.mensaje,
            suggestion: 'No se pudo extraer la información. Puedes ingresar los datos manualmente.',
        },
        error_desconocido: {
            ...
            >
                <Box>
                    ...
                    <Typography
                        ...
                        sx={{ mt: 0.5 }}
                    >
                        Sube tus facturas PDF para extraer y gestionar los datos automáticamente
                    </Typography>
                </Box>
                ...
                {estado !== 'idle' && (
                    <Chip
                        icon={estado === 'completado' ? <CheckCircle /> : <CloudUpload />}
                        ...
                        <ProcessingFeedback
                            message={
                                progress < 30
                                    ...
                                    : 'Validando información extraída...'
                            }
                            progress={progress}
                        />
                    </Paper>
