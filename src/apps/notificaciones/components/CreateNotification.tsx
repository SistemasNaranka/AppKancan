...
import {
  ...
  ToggleButton, ToggleButtonGroup, Divider, Paper, Collapse,
} from '@mui/material';
...
interface CreateNotificationProps {
  /** Callback para refrescar la tabla del historial tras enviar */
  onSuccess?: () => void;
}
...
const inputHoraStyle = {
  ...inputStyle,
  width: 175,
  '& .MuiOutlinedInput-input': { padding: '12px 6px', color: '#191b23', fontFamily: 'Inter' },
};
...
const CreateNotification = ({ onSuccess }: CreateNotificationProps) => {
  ...
  const agregarDestinatario = (valor: string) => {
    const v = valor.trim();
    if (v && !destinatarios.includes(v)) setDestinatarios([...destinatarios, v]);
    setInputDestinatario('');
  };

  const handleKeyDownDestinatario = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); agregarDestinatario(inputDestinatario); }
  };
  ...
  const resetForm = () => {
    setDestinatarios([]); setTitulo(''); setMensaje('');
    setProgramar(false); setFechaProgramada(''); setHoraProgramada('');
    setRecordatorio(false); setFechaRecordatorio(''); setHoraRecordatorio('');
    setPersistente(false); setClickeable(false); setRutaAccion('');
    setExcluir([]); setDuracionSeg(15);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setExito(true);
      resetForm();
      // ← Llama al callback para refrescar la tabla del historial
      onSuccess?.();
    } catch (err: any) {
    ...
    <>
      ...
      <Box sx={{ width: '100%', maxWidth: 1060, mx: 'auto', bgcolor: '#ffffff', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', border: '1px solid #e1e2ec', overflow: 'hidden' }}>
        ...
        <Box sx={{ p: 4, display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {/* Fila tarjetas superiores */}
          <Box sx={{ display: 'flex', gap: 3, width: '100%', alignItems: 'flex-start' }}>

            {/* Tarjeta: Programar */}
            <Paper elevation={0} sx={{ p: '12px 16px', bgcolor: '#f8f9fa', border: '1px solid #e5e7eb', borderRadius: '14px', flex: 1, display: 'flex', flexDirection: 'column', transition: 'all 0.3s ease', opacity: recordatorio ? 0.4 : 1, pointerEvents: recordatorio ? 'none' : 'auto' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                <Box sx={{ pr: 2 }}>
                  <Typography sx={{ fontSize: '15px', fontWeight: 600, color: '#191b23', fontFamily: 'Inter' }}>Programar para más tarde</Typography>
                  <Typography sx={{ fontSize: '12px', color: '#4b5563', fontFamily: 'Inter', mt: 0.5, lineHeight: 1.4 }}>Selecciona la fecha y hora de envío</Typography>
                </Box>
                <Switch checked={programar} onChange={(e) => setProgramar(e.target.checked)} disabled={recordatorio}
                ...
                <Box sx={{ display: 'flex', gap: 1.5, mt: 2 }}>
                  <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                    <DatePicker
                      ...
                      onChange={(v) => setFechaProgramada(v ? dayjs(v).format("YYYY-MM-DD") : "")}
            ...
            <Paper elevation={0} sx={{ p: '12px 16px', bgcolor: '#f8f9fa', border: '1px solid #e5e7eb', borderRadius: '14px', flex: 1, display: 'flex', flexDirection: 'column', transition: 'all 0.3s ease', opacity: programar ? 0.4 : 1, pointerEvents: programar ? 'none' : 'auto' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                <Box sx={{ pr: 2 }}>
                  <Typography sx={{ fontSize: '15px', fontWeight: 600, color: '#191b23', fontFamily: 'Inter' }}>Recordatorio personal</Typography>
                  <Typography sx={{ fontSize: '12px', color: '#4b5563', fontFamily: 'Inter', mt: 0.5, lineHeight: 1.4 }}>Envíate un recordatorio en una fecha y hora.</Typography>
                </Box>
                <Switch checked={recordatorio} onChange={(e) => { setRecordatorio(e.target.checked); if (e.target.checked) setProgramar(false); }} disabled={programar}
                  sx={{ '& .MuiSwitch-switchBase.Mui-checked': { color: '#0058be' }, '& .MuiSwitch-switchBase.Mui-checked + .MuiSwitch-track': { bgcolor: '#0058be' } }} />
              </Box>
                ...
                <Box sx={{ display: 'flex', gap: 1.5, mt: 2 }}>
                  <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                    <DatePicker
                      ...
                      onChange={(v) => setFechaRecordatorio(v ? dayjs(v).format("YYYY-MM-DD") : "")}
                      slotProps={{ textField: { size: "small", fullWidth: true, sx: inputStyle } }}
                    />
                  </LocalizationProvider>
          ...
          <Box sx={{ opacity: recordatorio ? 0.5 : 1, transition: 'opacity 0.3s', pointerEvents: recordatorio ? 'none' : 'auto' }}>
            <Typography sx={{ fontSize: '14px', fontWeight: 500, color: '#374151', fontFamily: 'Inter', mb: 1 }}>Destinatarios</Typography>
          ...
          <Box>
            <Box sx={{ position: 'relative' }}>
              <TextField fullWidth multiline rows={4} placeholder="Escribe el mensaje o descripción..." value={mensaje}
                ...
                sx={{ ...inputStyle, '& .MuiOutlinedInput-input': { padding: '4px 2px' } }}
              />
            ...
            <ToggleButtonGroup value={tipo} exclusive onChange={(_, val) => val && setTipo(val)} sx={{ flexWrap: 'wrap', gap: 2 }}>
              {tiposAlerta.map((t) => (
                <ToggleButton key={t.value} value={t.value}
                  sx={{ borderRadius: '999px !important', border: '1px solid #c2c6d6 !important', px: 2.5, py: 0.8, display: 'flex', gap: 0.75, textTransform: 'none', fontFamily: 'Inter', fontSize: '13.5px', fontWeight: 500, color: '#424754', transition: 'all 0.2s ease',
                    '&.Mui-selected': { bgcolor: `${t.color}24`, borderColor: `${t.color} !important`, color: t.color, boxShadow: `0 3px 10px ${t.color}30`, '&:hover': { bgcolor: `${t.color}35` } },
          ...
          <Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3.5 }}>
              ...
              <Box sx={{ opacity: persistente ? 0.5 : 1, transition: 'opacity 0.3s', pointerEvents: persistente ? 'none' : 'auto' }}>
                <Typography sx={{ fontSize: '13px', fontWeight: 500, color: '#424754', fontFamily: 'Inter', mb: 1.5 }}>Duración en pantalla</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', border: '1px solid #c2c6d6', borderRadius: '999px', overflow: 'hidden', bgcolor: '#f9f9ff' }}>
                    <Button onClick={() => setDuracionSeg(Math.max(5, duracionSeg - 5))} disabled={persistente}
                      sx={{ minWidth: 38, height: 38, borderRadius: 0, color: '#424754', fontSize: '18px', fontWeight: 300, '&:hover': { bgcolor: '#ecedf8' } }}>−</Button>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, px: 1 }}>
                      <input type="number" className="no-spin" value={duracionSeg}
                        onChange={(e) => setDuracionSeg(Math.min(360, Math.max(5, Number(e.target.value))))}
                        disabled={persistente}
                        style={{ width: 48, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'Inter', fontSize: '14px', fontWeight: 600, color: '#191b23', textAlign: 'center' }} />
                  ...
                  {duracionSeg === 15 && (
                    <Typography sx={{ fontSize: '11px', color: '#0058be', fontFamily: 'Inter', fontWeight: 500 }}>Recomendado</Typography>
                  )}
                </Box>
              </Box>
