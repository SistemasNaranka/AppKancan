            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
              {reservas.map(r => (
                <Box key={r.id} onClick={(e) => { onClose(); onSelectReserva(e, r); }} sx={{ p: 1.5, borderRadius: 1.5, border: "1px solid #e0e0e0", borderLeft: `4px solid ${getReservaColor(r.id)}`, cursor: "pointer", "&:hover": { backgroundColor: "#f9fafb" }, display: "flex", alignItems: "center", gap: 1.5 }}>
                  {r.estadoCalculado?.toLowerCase() === "en curso" && <PulsatingMeetingIndicator meetingDate={r.fecha} startTime={r.hora_inicio} endTime={r.hora_final} size={8} color="success" />}
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{r.titulo_reunion || "Sin título"}</Typography>
                    <Typography variant="caption" color="text.secondary">{formatearHora(r.hora_inicio)} - {formatearHora(r.hora_final)}</Typography>
                  </Box>
                </Box>
              ))}
            </Box>
  ...
  return (
    <Popover open={open} anchorEl={anchor} onClose={onClose} anchorOrigin={{ vertical: "center", horizontal: "right" }} transformOrigin={{ vertical: "center", horizontal: "left" }} PaperProps={{ sx: { width: 340, borderRadius: 2, boxShadow: "0 10px 40px rgba(0,0,0,0.15)" } }}>
      <Box sx={{ p: 2, backgroundColor: color, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Box sx={{ flex: 1, pr: 1 }}>
          <Typography variant="subtitle1" sx={{ fontWeight: 600, color: "#fff", lineHeight: 1.3 }}>{reserva.titulo_reunion || "Sin título"}</Typography>
          <Chip label={reserva.estadoCalculado || reserva.estado} size="small" sx={{ mt: 0.5, backgroundColor: "rgba(255,255,255,0.25)", color: "#fff", fontWeight: 600, fontSize: "0.7rem", height: 20 }} />
        </Box>
      ...
      <Box sx={{ p: 2 }}>
        {[
          { icon: <TimeIcon />, text: `${formatearHora(reserva.hora_inicio)} - ${formatearHora(reserva.hora_final)}` },
          { icon: <RoomIcon />, text: reserva.nombre_sala },
          { icon: <PersonIcon />, text: reserva.usuario_id ? `${reserva.usuario_id.first_name} ${reserva.usuario_id.last_name}` : "" },
          { icon: <AreaIcon />, text: reserva.area ? capitalize(reserva.area) : "" }
        ...
        {reserva.observaciones && (
          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5, mt: 2, pt: 2, borderTop: "1px solid #e0e0e0" }}>
            ...
            <Typography variant="body2" sx={{ color: "#374151", whiteSpace: "pre-wrap" }}>{reserva.observaciones}</Typography>
          </Box>
        )}
      </Box>
    </Popover>
