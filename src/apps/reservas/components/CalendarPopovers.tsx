...
import { Reservation, getReservationColor, capitalize } from "../types/reservas.types";
...
export const DayPopover: React.FC<PopoverProps & { 
  ...
  sala: string, 
  onNueva: (d: Date) => void,
  onSelectReserva: (e: React.MouseEvent<HTMLElement>, r: Reservation) => void 
...
export const DetailPopover: React.FC<PopoverProps & { 
  ...
  puedeModificar: (r: Reservation) => boolean,
  onEdit?: (r: Reservation) => void,
  onCancel?: (r: Reservation) => void
}> = ({ open, anchor, onClose, reserva, puedeModificar, onEdit, onCancel }) => {
  if (!reserva) return null;
  const color = getReservationColor(reserva.id);
  return (
    <Popover open={open} anchorEl={anchor} onClose={onClose} anchorOrigin={{ vertical: "center", horizontal: "right" }} transformOrigin={{ vertical: "center", horizontal: "left" }} PaperProps={{ sx: { width: 340, borderRadius: 2, boxShadow: "0 10px 40px rgba(0,0,0,0.15)" } }}>
      <Box sx={{ p: 2, backgroundColor: color, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Box sx={{ flex: 1, pr: 1 }}>
          ...
          <Chip label={reserva.calculatedStatus || reserva.status} size="small" sx={{ mt: 0.5, backgroundColor: "rgba(255,255,255,0.25)", color: "#fff", fontWeight: 600, fontSize: "0.7rem", height: 20 }} />
        </Box>
      ...
      <Box sx={{ p: 2 }}>
        {[
          { icon: <TimeIcon />, text: `${formatTime(reserva.start_time)} - ${formatTime(reserva.end_time)}` },
