...
const EstadoSalas: React.FC<EstadoSalasProps> = ({
  ...
  const estadosSalas = useMemo((): EstadoSala[] => {
    ...
    return SALAS_DISPONIBLES.map((sala) => {
      ...
      const hayReservasHoy = reservasSalaHoy.some((r) => r.hora_inicio > horaActual || (r.hora_inicio <= horaActual && r.hora_final > horaActual));
      ...
      if (hayReservasHoy) {
        ...
        const reservasFuturas = reservasSala
          .filter((r) => r.fecha > hoy || (r.fecha === hoy && r.hora_inicio > horaActual))
          .sort((a, b) => {
      ...
      return {
        ...
        proximaReserva: proximasReservas.length > 0 ? proximasReservas[0] : null,
        proximasReservas,
        esReservaFutura,
      };
    });
  ...
  const obtenerEtiquetaFechaReserva = (reserva: Reserva | null, esFutura: boolean): string => {
    if (!reserva) return "";
