...
import { obtenerTiendas, obtenerEmpleadosPorFechaExacta, obtenerAsesores, obtenerCargos, obtenerPresupuestosDiarios, obtenerPorcentajesMensuales } from "../api/directus/read";
...
export const EditStoreModalSimplified: React.FC<EditStoreModalSimplifiedProps> = ({
    isOpen,
    onClose,
    onSaveComplete,
    selectedMonth,
    ...
    return (
        <>
            ...
            >
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                        ...
                        <Box>
                            <Typography variant="h5" fontWeight="700">
                                {tiendaNombre ? `Editar Asignación - ${tiendaNombre} ` : "Editar Asignación"}
                            </Typography>
                    ...
                    <Box sx={{
                        ...
                        <Grid container spacing={3}>
                            <Grid size={{ xs: 12, md: 6 }}>
                                ...
                                <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="es">
                                    <DatePicker
                                        value={dayjs(fecha)}
                                        format="DD/MM/YYYY"
                                        onChange={(newValue) => setFecha(newValue ? newValue.format("YYYY-MM-DD") : "")}
                                        slotProps={{
                                            textField: {
                                                fullWidth: true,
                                                size: "medium",
                            ...
                            <Grid size={{ xs: 12, md: 6 }}>
                                ...
                                <Autocomplete
                                    options={tiendas}
                                    getOptionLabel={(option) => `${option.nombre} - ${option.empresa}`}
                                    value={tiendas.find(t => t.id === tiendaSeleccionada) || null}
                                    onChange={(_, newValue) => handleTiendaChange(newValue ? newValue.id : 0)}
                                    renderInput={(params) => (
                                        <TextField
                                            {...params}
                    ...
                    <Box sx={{
                        ...
                        <Grid container spacing={2} alignItems="flex-end">
                            ...
                            <Grid item xs={12} sm={6} md={2.5}>
                                <Typography variant="caption" sx={{ mb: 0.5, fontWeight: 600, color: "text.secondary", display: "block" }}>
                                    Cargo del Día
                                </Typography>
                                ...
                                {empleadoEncontrado ? (
                                    <Box sx={{
                                        ...
                                        <Box sx={{
                                            ...
                                            <Typography variant="caption" fontWeight="600" color="#2e7d32" sx={{ fontSize: "0.7rem" }}>
                            ...
                            <Grid item xs={12} md={2}>
                                <Button
                                    ...
                                    disabled={!tiendaSeleccionada || !empleadoEncontrado || !cargoSeleccionado}
                                    sx={{
                                        py: 1,
                                        fontSize: "0.85rem",
                                        fontWeight: 700,
                        ...
                        <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: "block", fontStyle: "normal", textAlign: "left", pl: 1 }}>
                            Ingrese el código de 4 dígitos del empleado y seleccione el cargo correspondiente. El sistema validará automáticamente la información antes de agregarlo a la lista.
                        </Typography>
                    </Box>
                                    ...
                                    <Grid size={{ xs: 12, sm: 6, md: 3 }} key={empleado.id}>
                                        <Card sx={{
                                            ...
                                            <CardContent sx={{ p: 1.5, "&:last-child": { pb: 1.5 } }}>
                                                <Typography variant="subtitle2" fontWeight="bold" sx={{ pr: 3, mb: 0.5, lineHeight: 1.2, fontSize: "1rem" }}> {/* Increased Name Size slightly */}
                                                    {empleado.nombre}
                                                </Typography>
                                                <Typography
                                                    variant="body2" // Increased from caption
                                                    sx={{
                                                        color: empleado.cargo_nombre === "Gerente" ? "success.main" : "primary.main",
                                                ...
                                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pt: 1, borderTop: "1px solid", borderColor: "grey.200" }}>
                                                    <Typography variant="caption" color="text.secondary" fontWeight="600" sx={{ fontSize: "0.8rem" }}> {/* Increased Code Size */}
                                                        Cod. {empleado.codigo}
                                                    </Typography>
                                                    <Typography
                                                        variant="body2"
                ...
                <DialogActions sx={{ p: 2.5, borderTop: "1px solid #e0e0e0", bgcolor: "#f5f5f5", gap: 1.5 }}>
                    <Button
                        onClick={onClose}
                        disabled={loading}
                        variant="outlined"
                    ...
                    <Button
                        ...
                        disabled={loading || !tiendaSeleccionada || empleadosAsignados.length === 0}
                        sx={{
                            minWidth: 200,
                            fontWeight: 600,
                            boxShadow: 2,
                    ...
                    >
                        {loading ? "Guardando..." : `Actualizar Asignación(${empleadosAsignados.length} empleados)`}
                    </Button>
                </DialogActions>
            </Dialog >
        </>
