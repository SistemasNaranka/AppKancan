...
export async function obtenerPromociones(): Promise<Promotion[]> {
  try {
    ...
    const promos = await withAutoRefresh(()=>directus.request(
      readItems("promo", {
        fields: [
          "id",
          "nombre",
    ...
    promoTiendas.forEach((pt: any) => {
      const promoId = typeof pt.promo_id === "number" ? pt.promo_id : pt.promo_id?.id;
 ...
 */
export async function obtenerPromocionPorId(id: number): Promise<Promotion | null> {
  try {
    // 1. Obtener la promoción con el tipo expandido
    const promo = await withAutoRefresh(()=>directus.request(
      readItems("promo", {
        fields: [
          "id",
          "nombre",
    ...
    const promoTiendas = await withAutoRefresh(()=>directus.request(
      readItems("promo_tiendas", {
    ...
    const tiendas = promoTiendas.map((pt: any) => pt.tiendas_id?.nombre || "Sin nombre");
 ...
 */
export async function obtenerTiendasDePromocion(promoId: number): Promise<DirectusTienda[]> {
  try {
    const promoTiendas = await directus.request(
      readItems("promo_tiendas", {
        fields: [
