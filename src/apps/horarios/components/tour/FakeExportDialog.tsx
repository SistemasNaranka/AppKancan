...
interface FakeExportDialogProps {
  open: boolean;
  activeField: string | null;
  title: string;
  subtitle: string;
  idPrefix: string;
  showDetalladaOption?: boolean;
  maxWidth?: number;
}

export const FakeExportDialog: React.FC<FakeExportDialogProps> = ({
  open,
  activeField,
  title,
  subtitle,
  idPrefix,
  showDetalladaOption = false,
  maxWidth = 440,
}) => {
  if (!open) return null;

  const fechaId = `#${idPrefix}-fecha-field`;
  const detalladaId = `#${idPrefix}-detallada`;
  const botonId = `#${idPrefix}-boton`;

  const isFechaActive = activeField === fechaId;
  const isDetalladaActive = activeField === detalladaId;
  const isBotonActive = activeField === botonId;
  const isModalStep = isFechaActive || isDetalladaActive || isBotonActive;
    ...
    >
      ...
      >
        ...
        <div style={{ padding: 20, position: "relative" }}>
          ...
          <div style={{ display: "flex", gap: 10, backgroundColor: "#fefce8", border: "1px solid #fde68a", borderRadius: 8, padding: "12px 14px", marginBottom: showDetalladaOption ? 16 : 20 }}>
          ...
          {showDetalladaOption && (
            <div
              ...
              style={{
                ...
                boxShadow: isDetalladaActive ? "0 0 8px rgba(0, 140, 255, 0.35)" : "none",
                transition: "all 0.1s ease",
              }}
            >
              <input
...
export const buildExportModalTargets = (idPrefix: string, includeDetallada = false): string[] => {
  const targets = [`#${idPrefix}-fecha-field`, `#${idPrefix}-boton`];
  if (includeDetallada) targets.splice(1, 0, `#${idPrefix}-detallada`);
  return targets;
};