...
import {
  ...
  TableContainer, TableHead, TableRow, Chip, CircularProgress, Alert, Pagination,
  Tabs, Tab, Grid, TextField, InputAdornment, Dialog, DialogTitle, DialogContent,
  DialogActions, IconButton, Tooltip, FormControl, InputLabel, Select, MenuItem,
  OutlinedInput, Avatar, List, ListItem, ListItemAvatar, ListItemText,
  ListItemSecondaryAction
} from '@mui/material';
import {
  Storefront as StorefrontIcon, Person as PersonIcon, CheckCircle as CheckCircleIcon,
  Pending as PendingIcon, Visibility as VisibilityIcon, EditNote as EditNoteIcon,
  Warning as WarningIcon, SupervisorAccount as SupervisorAccountIcon, Search as SearchIcon,
  Close as CloseIcon, Assignment as AssignmentIcon, Message as MessageIcon,
  ArrowUpward as ArrowUpwardIcon, ArrowDownward as ArrowDownwardIcon,
  CalendarToday as CalendarTodayIcon, ArrowBackIos as ArrowBackIosIcon,
  ArrowForwardIos as ArrowForwardIosIcon, Today as TodayIcon, Sort as SortIcon
...
const StatCard = ({ icon: Icon, label, value, color, bg, onClick }: any) => (
  <Paper elevation={0} onClick={onClick} sx={{ p: 2.5, borderRadius: 3, border: '1px solid #e0e0e0', display: 'flex', alignItems: 'center', gap: 2, cursor: onClick ? 'pointer' : 'default', transition: 'all 0.2s', '&:hover': onClick ? { borderColor: '#004680', boxShadow: '0 2px 8px rgba(0,70,128,0.15)' } : {} }}>
    <Box sx={{ bgcolor: bg, p: 1.5, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon sx={{ color, fontSize: 28 }} /></Box>
    <Box sx={{ minWidth: 0, width: '100%' }}>
      <Typography variant="caption" color="text.secondary" display="block" fontWeight={600}>{label}</Typography>
      <Typography variant={label === 'TIENDA CON MÁS CAMBIOS' ? 'h6' : 'h5'} fontWeight={700} color={color} noWrap title={value}>{value}</Typography>
    </Box>
  </Paper>
);
...
=======
    ...
    const rowsPerPage = { tiendas: 10, ediciones: 10, ranking: 10 };
>>>>>>> 26c00a419ac2377edb8a47def6bd015be1c96400
  ...
  const motivosUnicos = useMemo(() => [...new Set(editedRecords.map(r => r.motivo).filter(Boolean))].sort(), [editedRecords]);
...
<<<<<<< HEAD
    let topStoreName = 'Ninguna';
    let topStoreCount = 0;
    Object.entries(storeCounts).forEach(([name, count]) => {
      if (count > topStoreCount) {
        topStoreCount = count;
        topStoreName = name;
      }
=======
...
>>>>>>> 26c00a419ac2377edb8a47def6bd015be1c96400
    ...
    const topStoreDisplay = topStoreCount > 0 ? `${topStoreName} (${topStoreCount})` : 'Ninguna (0)';
    return { total: editedRecords.length, empleados: uniqueEmpleados.size, topStore: topStoreDisplay };
  }, [editedRecords]);

<<<<<<< HEAD
  // 👇 RANKING BASE MODIFICADO PARA INCLUIR LA TIENDA
    ...
    editedRecords.forEach(r => {
      if (!map.has(r.empleadoId)) {
        map.set(r.empleadoId, {
          ...
          total: 0,
          tienda: r.tiendaNombre, // Se asigna la tienda del primer registro
        });
      }
...
>>>>>>> 26c00a419ac2377edb8a47def6bd015be1c96400
  ...
  const rankingEmpleados = useMemo(() => [...rankingBase].sort((a, b) => ordenEdiciones === 'asc' ? a.total - b.total : b.total - a.total), [rankingBase, ordenEdiciones]);
  const rankingFiltrado = useMemo(() => {
    const q = buscarEmpleadoRanking.trim().toLowerCase();
    return q ? rankingEmpleados.filter(e => e.nombre.toLowerCase().includes(q)) : rankingEmpleados;
...
>>>>>>> 26c00a419ac2377edb8a47def6bd015be1c96400
  ...
  const diasConEdiciones = useMemo(() => new Set(edicionesPorEmpleado.map(r => r.fecha)), [edicionesPorEmpleado]);
  ...
  const conteoPorDia = useMemo(() => edicionesPorEmpleado.reduce((acc, r) => ({ ...acc, [r.fecha]: (acc[r.fecha] || 0) + 1 }), {} as Record<string, number>), [edicionesPorEmpleado]);
  ...
  const diasConEdicionesMes = useMemo(() => new Set(edicionesDelMes.map(r => r.fecha)), [edicionesDelMes]);
  const promedioEdicionesMes = useMemo(() => diasConEdicionesMes.size > 0 ? edicionesDelMes.length / diasConEdicionesMes.size : 0, [edicionesDelMes, diasConEdicionesMes]);
            ...
            <Container maxWidth="xl" disableGutters>
                ...
                <Dialog open={modalEmpleadosOpen} onClose={() => setModalEmpleadosOpen(false)} maxWidth="lg" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                    ...
                    <DialogContent sx={{ p: 3, pt: 1 }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 3, mt: 1.5 }}>
                    ...
                    <DialogContent sx={{ p: 3, bgcolor: '#f5f7fa' }}>
                        ...
                        <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: '1px solid #e0e0e0', bgcolor: '#ffffff' }}>
                            ...
                            <Grid container spacing={1}>
                                {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map(d => <Grid size="grow" key={d}><Typography align="center" variant="caption" fontWeight={700} color="text.secondary">{d}</Typography></Grid>)}
                            </Grid>
                            <Grid container spacing={1} sx={{ mt: 1 }}>
                                {Array.from({ length: (calendario.mes.startOf('month').day() + 6) % 7 }, (_, i) => <Grid size="grow" key={`empty-${i}`} />)}
...
>>>>>>> 26c00a419ac2377edb8a47def6bd015be1c96400
  ...
  if (errorResumen) {
    return (
      <Container>
        <Alert severity="error" action={<Button color="inherit" size="small" onClick={() => {}}>Reintentar</Button>}>
          {errorResumen}
        </Alert>
      </Container>
    );
  ...
  return (
    <Box sx={{ backgroundColor: 'transparent', py: 2 }}>
      <Container maxWidth="xl" disableGutters>
        ...
        {subTab === 0 && (
          <>
            ...
            <Paper elevation={0} sx={{ p: 1, mb: 2, borderRadius: 2, border: '1px solid #e0e0e0', display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', bgcolor: '#f8fafc' }}>
              ...
              <FormControl size="small" sx={{ minWidth: 140 }}>
                <Select value={ordenTiendas.by} onChange={(e) => handleOrdenTiendas(e.target.value as SortField)} variant="outlined" size="small" sx={{ bgcolor: '#fff', borderRadius: 1.5, '& .MuiSelect-select': { py: 0.5, fontSize: '0.8rem' } }}>
                  {OPCIONES_ORDEN.map(opt => <MenuItem key={opt.value} value={opt.value} sx={{ fontSize: '0.8rem' }}>{opt.label}</MenuItem>)}
                </Select>
              </FormControl>
            </Paper>
            ...
            <TableContainer component={Paper} elevation={0} sx={{ borderRadius: 3, border: '1px solid #e0e0e0' }}>
              <Table size="medium">
                <TableHead sx={{ bgcolor: '#f8fafc' }}>
                  <TableRow>
                    {COLUMNAS_TIENDAS.map(col => (
                      <TableCell key={col.label} sx={{ fontWeight: 700, color: '#546e7a', cursor: col.value ? 'pointer' : 'default', userSelect: 'none', '&:hover': col.value ? { color: '#004680' } : {} }} align={col.label === 'ACCIONES' ? 'center' : 'left'} onClick={() => col.value && handleOrdenTiendas(col.value)}>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: col.label === 'ACCIONES' ? 'center' : 'flex-start' }}>
                          {col.label}
                          {ordenTiendas.by === col.value && (ordenTiendas.dir === 'asc' ? <ArrowUpwardIcon sx={{ fontSize: '1rem', color: '#004680' }} /> : <ArrowDownwardIcon sx={{ fontSize: '1rem', color: '#004680' }} />)}
                        </Box>
                      </TableCell>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {tiendasPagina.length === 0 ? <TableRow><TableCell colSpan={6} align="center" sx={{ py: 4, color: 'text.secondary' }}>No hay tiendas disponibles</TableCell></TableRow> : tiendasPagina.map(tienda => (
                    <TableRow key={tienda.id} hover>
                      <TableCell><Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}><StorefrontIcon sx={{ color: '#004680' }} /><Typography fontWeight={600} color="#0a1929">{tienda.nombre}</Typography></Box></TableCell>
                      <TableCell align="center"><Chip size="small" label={tienda.totalEmpleados} sx={{ bgcolor: '#e3f2fd', color: '#0a1929', fontWeight: 700 }} /></TableCell>
                      <TableCell align="center"><Chip size="small" label={tienda.personasRegistradas} sx={{ bgcolor: '#ede7f6', color: '#5e35b1', fontWeight: 700 }} /></TableCell>
                      <TableCell align="center"><Chip size="small" label={tienda.completados} sx={{ bgcolor: '#e8f5e9', color: '#2e7d32', fontWeight: 700 }} /></TableCell>
                      <TableCell align="center"><Chip size="small" label={tienda.pendientes} sx={{ bgcolor: '#fbe9e7', color: '#d32f2f', fontWeight: 700 }} /></TableCell>
                      <TableCell align="center"><Button variant="contained" size="small" startIcon={<VisibilityIcon />} onClick={() => setTiendaSeleccionada(tienda.id)} sx={{ bgcolor: '#004680', '&:hover': { bgcolor: '#003a6b' }, textTransform: 'none', borderRadius: 2 }}>Ver empleados</Button></TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          <>
            <Grid container spacing={3} sx={{ mb: 3 }}>
              {TARJETAS_ESTADISTICAS.map((k, idx) => (
                <Grid item xs={12} md={3} key={idx}>
                  <StatCard icon={k.icon} label={k.label} value={k.label === 'TOTAL MODIFICACIONES' ? statsEdiciones.total : k.label === 'EMPLEADOS MONITOREADOS' ? statsEdiciones.empleados : statsEdiciones.topStore} color={k.color} bg={k.bg} />
                </Grid>
              ))}
              <Grid item xs={12} md={3}>
                <StatCard icon={PersonIcon} label="EMPLEADO CON MÁS EDICIONES" value={topEmpleado} color="#004680" bg="#e3f2fd" onClick={() => setModalEmpleadosOpen(true)} />
              </Grid>
            </Grid>
            ...
            <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 3, border: '1px solid #e0e0e0', display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <TextField
                ...
                InputProps={{
                  startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" sx={{ color: '#94a3b8' }} /></InputAdornment>,
                  endAdornment: buscarEmpleado ? <InputAdornment position="end"><IconButton size="small" onClick={() => { setBuscarEmpleado(''); setPaginaEdiciones(0); }}><CloseIcon fontSize="small" /></IconButton></InputAdornment> : null,
                }}
              />
            </Paper>
        ...
        {tiendaSeleccionada && <ModalDetalleTienda tiendaId={tiendaSeleccionada} tiendaNombre={tiendas.find(t => t.id === tiendaSeleccionada)?.name || 'Detalle de Tienda'} onClose={() => setTiendaSeleccionada(null)} />}
        ...
        {/* ----- MODAL DE RANKING (con la tienda añadida) ----- */}
        <Dialog open={modalEmpleadosOpen} onClose={() => setModalEmpleadosOpen(false)} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
          ...
          <DialogContent sx={{ p: 3, pt: 2 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', mb: 3, mt: 4 }}>
              <TextField
                label="Buscar empleado"
                placeholder="Escribe el nombre del empleado..."
