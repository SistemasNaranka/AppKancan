...
export const ContratoForm: React.FC<ContratoFormProps> = ({
  ...
  useEffect(() => {
    if (open && initialData) {
      ...
      if (
        initialData.empleado_area &&
        !AREAS_PREDEFINIDAS.includes(initialData.empleado_area)
      ) {
        setAreasExtra((prev) =>
          prev.includes(initialData.empleado_area) ? prev : [...prev, initialData.empleado_area],
        );
      }
