        ...
        <Grid size={{ xs: 12, md: 8 }}>
          <Stack spacing={2.5}>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              {paginatedProrrogas.length === 0 ? (
                <Box sx={{ py: 6, textAlign: 'center', color: 'text.secondary' }}>
                  <Typography variant="body2">No hay prórrogas registradas.</Typography>
                </Box>
              ) : (
                <>
                  {paginatedProrrogas.map((p, idx) => {
                    const globalReverseIdx = (page - 1) * itemsPerPage + idx;
                    const isChronologicallyOldest = globalReverseIdx === prorrogas.length - 1;
                    const isChronologicallyNewest = globalReverseIdx === 0;
                    
                    return (
                      <TimelineEntry
                        key={p.id}
                        prorroga={p}
                        displayIndex={globalReverseIdx}
                        isLast={isChronologicallyOldest}
                        isActive={isChronologicallyNewest && !isVencido}
                      />
                    );
                  })}
                  {totalPages > 1 && (
