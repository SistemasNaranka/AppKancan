...
import { diasRestantes } from '../../../services/directus/resolucionesApi';
...
const ResolutionCard: React.FC<ResolutionCardProps> = ({
  ...
  const infoVencimiento = resolucion?.vigencia && resolucion?.fecha_creacion
...
const campos = [
  { etiqueta: 'Resolución', valor: formatearResolucion(resolucion?.numero_formulario) },
  ...
  { etiqueta: 'Vigencia', valor: resolucion?.vigencia ? `${resolucion.vigencia} meses` : '' },
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Box>
                {campos.map((campo) => {
                    ...
                    return (
                      <Box key={campo.etiqueta} sx={{ display: 'flex', mb: 1.5, alignItems: 'flex-start' }}>
                        <Typography sx={{ fontWeight: 'bold', minWidth: 110, flexShrink: 0 }}>
                          {campo.etiqueta}:
                        </Typography>
                        <Typography sx={{ 
                          color: colorTexto, 
                          fontWeight: campo.etiqueta === 'Vencimiento' ? 'bold' : 'normal',
