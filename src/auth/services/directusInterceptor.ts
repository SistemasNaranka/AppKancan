import { cargarTokenStorage, guardarTokenStorage,isExpired, borrarTokenStorage } from "@/auth/services/tokenDirectus";
...
export async function ensureValidToken(): Promise<void> {
  ...
  if (isExpired(tokens.expires_at)) {
    ...
    try {
      ...
      guardarTokenStorage(newTokens.access_token, newTokens.refresh_token, newTokens.expires_at);
 ...
 * ```
 */
export async function withAutoRefresh<T>(directusRequest: () => Promise<T>): Promise<T> {
